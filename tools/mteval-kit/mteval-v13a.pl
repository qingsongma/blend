...
sub score_document
{
   ...
   #score each segment
	foreach my $seg ( sort{ $a <=> $b } keys( %{$tst_data{$sys}{$doc}{SEGS}} ) )
	{
        ...
        # ---------------------------------------------------
        #this piece of code has been MODIFIED by Asiya developers
#		if ( $method eq "BLEU" )
#		{
#			my %DOCmt = ();
...
#		}
		...
		if ( ( $detail >= 2 ) && $method eq "BLEU" )
		{
         ...
         printf "  cumulative-$method score using 4-grams = %.4f %.4f %.4f %.4f for system \"$sys\" on segment %d of document \"$doc\" (%d words)\n", 
               $DOCmt{1}{$sys}{cum}, $DOCmt{2}{$sys}{cum}, $DOCmt{3}{$sys}{cum}, $DOCmt{4}{$sys}{cum}, $seg, $tst_cnt->[1];
         printf "  individual-$method score using 4-grams = %.4f %.4f %.4f %.4f for system \"$sys\" on segment %d of document \"$doc\" (%d words)\n", 
               (exists($DOCmt{1}{$sys}{ind})? $DOCmt{1}{$sys}{ind} : 0), (exists($DOCmt{2}{$sys}{ind})? $DOCmt{2}{$sys}{ind} : 0), (exists($DOCmt{3}{$sys}{ind})? $DOCmt{3}{$sys}{ind} : 0), 
               (exists($DOCmt{4}{$sys}{ind})? $DOCmt{4}{$sys}{ind} : 0), $seg, $tst_cnt->[1];			
			$overallScore->{ $sys }{ 'documents' }{ $doc }{ 'segments' }{ $seg }{ 'score' } = $segScore;
         #printf "  $method score using 4-grams = %.4f for system \"$sys\" on segment $seg of document \"$doc\" (%d words)\n", $segScore, $tst_cnt->[1]
		}
		if ( ( $detail >= 2 ) && $method eq "NIST" )
		{
         ...
         printf "  cumulative-$method score using 5-grams = %.4f %.4f %.4f %.4f %.4f for system \"$sys\" on segment %d of document \"$doc\" (%d words)\n", 
                  $DOCmt{1}{$sys}{cum}, $DOCmt{2}{$sys}{cum}, $DOCmt{3}{$sys}{cum}, $DOCmt{4}{$sys}{cum}, $DOCmt{5}{$sys}{cum}, $seg, $tst_cnt->[1];
         printf "  individual-$method score using 5-grams = %.4f %.4f %.4f %.4f %.4f for system \"$sys\" on segment %d of document \"$doc\" (%d words)\n", 
                  (exists($DOCmt{1}{$sys}{ind})? $DOCmt{1}{$sys}{ind} : 0), (exists($DOCmt{2}{$sys}{ind})? $DOCmt{2}{$sys}{ind} : 0), (exists($DOCmt{3}{$sys}{ind})? $DOCmt{3}{$sys}{ind} : 0), 
                  (exists($DOCmt{4}{$sys}{ind})? $DOCmt{4}{$sys}{ind} : 0), (exists($DOCmt{5}{$sys}{ind})? $DOCmt{5}{$sys}{ind} : 0), $seg, $tst_cnt->[1];
			$overallScore->{ $sys }{ 'documents' }{ $doc }{ 'segments' }{ $seg }{ 'score' } = $segScore;
         #printf "  $method score using 5-grams = %.4f for system \"$sys\" on segment $seg of document \"$doc\" (%d words)\n", $segScore, $tst_cnt->[1];
		}
