...
#################################
# modifications:
#   * segment bleu scores (n=1..4) are printed
#   * segment nist scores (n=1..5) are printed
#
#################################
# History:
#
...
#x                  if $detail >= 2;
        ...
        # ---------------------------------------------------
        #this piece of code has been MODIFIED by IQMT developers
        if (($detail >=2) && ($METHOD eq "BLEU")) {
            my %DOCmt = ();
            bleu_score($shortest_ref_length, $match_cnt, $tst_cnt, $sys, \%DOCmt);
            printf "  cumulative-$method score using 4-grams = %.4f %.4f %.4f %.4f for system \"$sys\" on segment %d of document \"$doc\" (%d words)\n", $DOCmt{1}{$sys}{cum}, $DOCmt{2}{$sys}{cum}, $DOCmt{3}{$sys}{cum}, $DOCmt{4}{$sys}{cum}, $jseg+1, $tst_cnt->[1];
            printf "  individual-$method score using 4-grams = %.4f %.4f %.4f %.4f for system \"$sys\" on segment %d of document \"$doc\" (%d words)\n", (exists($DOCmt{1}{$sys}{ind})? $DOCmt{1}{$sys}{ind} : 0), (exists($DOCmt{2}{$sys}{ind})? $DOCmt{2}{$sys}{ind} : 0), (exists($DOCmt{3}{$sys}{ind})? $DOCmt{3}{$sys}{ind} : 0), (exists($DOCmt{4}{$sys}{ind})? $DOCmt{4}{$sys}{ind} : 0), $jseg+1, $tst_cnt->[1];
        }
        if (($detail >=2) && ($METHOD eq "NIST")) {
            my %DOCmt = ();
            nist_score(scalar @ref_sys, $match_cnt, $tst_cnt, $ref_cnt, $tst_info, $ref_info, $sys, \%DOCmt);
            printf "  cumulative-$method score using 5-grams = %.4f %.4f %.4f %.4f %.4f for system \"$sys\" on segment %d of document \"$doc\" (%d words)\n", $DOCmt{1}{$sys}{cum}, $DOCmt{2}{$sys}{cum}, $DOCmt{3}{$sys}{cum}, $DOCmt{4}{$sys}{cum}, $DOCmt{5}{$sys}{cum}, $jseg+1, $tst_cnt->[1];
            printf "  individual-$method score using 5-grams = %.4f %.4f %.4f %.4f %.4f for system \"$sys\" on segment %d of document \"$doc\" (%d words)\n", (exists($DOCmt{1}{$sys}{ind})? $DOCmt{1}{$sys}{ind} : 0), (exists($DOCmt{2}{$sys}{ind})? $DOCmt{2}{$sys}{ind} : 0), (exists($DOCmt{3}{$sys}{ind})? $DOCmt{3}{$sys}{ind} : 0), (exists($DOCmt{4}{$sys}{ind})? $DOCmt{4}{$sys}{ind} : 0), (exists($DOCmt{5}{$sys}{ind})? $DOCmt{5}{$sys}{ind} : 0), $jseg+1, $tst_cnt->[1];
        }
